...
#     result = np.dot(R, np.array(old)) + np.array(t)
#     return result

def turn(u, q, old, t ):
    u /= np.linalg.norm(u)
    R = Quaternion(np.cos(q/2), u[0]*np.sin(q/2), u[1]*np.sin(q/2), u[2]*np.sin(q/2))
    p = Quaternion(0, old[0], old[1], old[2])
    tt = Quaternion(0, t[0], t[1], t[2])
    result = R*p*Quaternion.conj(R) + tt
    return Quaternion.return_array(result)
...
vertices = []
with open("model1.obj", "r") as file:
    for line in file:
        if line.startswith("v "):
            data = line.split()
            x, y, z = map(float, data[1:])
            x, y, z = turn([1, 1, 0], np.pi, [x, y, z], [0, -0.02, 0.5])
...
polygons = read_model_data('model1.obj')
...
texture_picture = np.array(ImageOps.flip(Image.open('model1.jpg')))
    ...
    for x in range(xmin, xmax):
        for y in range(ymin, ymax):
            ...
            if b0 >= 0 and b1 >= 0 and b2 >= 0 and def_cos(x0, y0, z0, x1, y1, z1, x2, y2, z2) < 0:
                z = b0 * z0 + b1 * z1 + b2 * z2
                if z < z_buf[y][x]:
                    pixel = texture_picture[int(h * (b0 * v0 + b1 * v1 + b2 * v2)), int(w * (b0 * u0 + b1 * u1 + b2 * u2))]
                    c = np.dot(color, b)
                    if c > 0: c = 0
                    img[y][x] = pixel
                    z_buf[y][x] = z
...
textures = load_texture_indexes('model1.obj')
res_textures = load_texture_coords('model1.obj')
